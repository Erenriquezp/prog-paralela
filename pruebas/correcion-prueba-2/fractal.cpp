...
static uint32_t burning_ship_color(double cx, double cy, int& out_iter)
{
    double zr = 0.0;
    double zi = 0.0;
    int iter = 0;
                 ...
                 uint32_t* pixel_buffer, int* local_hist)
{
    ...
    for (int i = 0; i < NBINS; ++i) local_hist[i] = 0;
    ...
    for (uint32_t row = row_start; row < row_end; ++row) {
        ...
        for (uint32_t col = 0; col < width; ++col) {
            ...
            int iter = 0;
            pixel_buffer[(row - row_start) * width + col] = burning_ship_color(cx, cy, iter);
            if (iter < max_iteraciones) {
